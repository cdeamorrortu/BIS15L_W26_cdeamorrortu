...
```{r}
hot_springs$spring <- as.factor(hot_springs$springs)
...
```{r}
fish <- read_csv("data/Gaeta_etal_CLC_data.csv") #include "data/" b/c the fie is in a folder within a folder
```
...
`glimpfish`glimpse()` is another useful summary function.
```{r}
glimpse(fish)
```
