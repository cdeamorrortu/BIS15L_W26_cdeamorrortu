...
life_history %>% 
  ...
  labs(title = "Number of Observations of Each Mammal Order", x = "Order", y = "Count") +
  scale_y_log10() +
  coord_flip()
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean.mass = mean(mass, na.rm = T)) %>% 
...
life_history %>% 
  ...
  summarize(mean.mass = mean(mass, na.rm = T)) %>% 
  arrange(desc(mean.mass)) %>% 
  ggplot(mapping = aes(x = order, y = mean.mass)) +
  geom_col(mapping = aes(fill = order)) +
  labs(title = "Average Mass per Order", x = "Order", y = "Mean Mass") +
  scale_y_log10() +
  coord_flip()
```
...
life_history %>% 
  ...
  summarize(mean.mass = mean(mass, na.rm = T)) %>% 
  arrange(desc(mean.mass)) %>% 
  ggplot(mapping = aes(x = reorder(order, mean.mass), y = mean.mass)) +
  geom_col(mapping = aes(fill = order)) +
  labs(title = "Average Mass per Order", x = "Order", y = "Mean Mass") +
  scale_y_log10() +
  coord_flip()
...
life_history %>% 
  ...
  geom_boxplot(aes(color = order), na.rm = T) +
  coord_flip()
```
