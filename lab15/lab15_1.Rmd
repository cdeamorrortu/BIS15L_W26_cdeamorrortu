...
msleep %>% 
  ...
  ggplot(x = vore, y = sleep_total) +
  geom_boxplot(aes(x = vore, y = sleep_total, fill = vore)) +
  labs(title = "Sleep Variables by Vore Type", x = "Vore", y = "Sleep Total")
...
```{r, eval=F, echo=T}
library(shiny) #doesn't need to be here, can keep or delete since we already loaded shiny in
