...
\
```{r}
anolis <- anolis %>% mutate(across(c(habitat, island), as.factor))
anolis
```
...
anolis %>% 
  count(island) %>% 
  ggplot(mapping = aes(x = island, y = n)) +
  geom_col(mapping = aes(fill = island)) +
  labs(title = "Anole Species Sampled per Island", x = "Island", y = "Different Species")
```
...
anolis %>% 
  ...
  labs(title = "Number of Anole Species by Habitat", x = "Habitat", y = "Number of Species")
```
...
anolis %>% 
  ...
  labs(title = "Hindlimb Length by Habitat", x = "Habitat", y = "Hindlimb Length (mm)")
```
...
anolis %>% 
  ...
  geom_boxplot(mapping = aes(fill = habitat)) +
  labs(title = "Ratio of Anole Hindlimbs to Body Length by Habitat", x = "Habitat", y = "Ratio of Hindlimb Length to Body Length")
```
...
anolis %>% 
  ...
  geom_point(na.rm = T, mapping = aes(color = habitat)) +
  geom_smooth(method = "lm", se = F) +
  labs(title = "Relationship Between Body Length (mm) and Tail Length(mm) by Habitat")
...
\
```{r}
anolis %>% distinct(habitat)
anolis %>% 
  filter(habitat == "Trunk-ground") %>% 
  summarize(habitat, toepad_lamellae_count, meantlc = mean(toepad_lamellae_count)) %>% 
  select(habitat, meantlc) %>% 
  distinct()
...
anolis %>% 
  filter(habitat == "Trunk-crown") %>% 
  select(island, toepad_lamellae_count) %>% 
  ggplot(mapping = aes(x = island, y = toepad_lamellae_count))+
  geom_boxplot(mapping = aes(fill = island)) +
  labs(title = "Range of Trunk-Crown Toepad Lamellae by Island", x = "Island", y = "Toepad Lamellae Count")
