...
```{r}
select(penguins, species:flipper_length_mm) #everything between the two variable columns listed, includes them
```

The ! operator is useful in select. It allows us to select everything except the specified variables.
```{r}
select(penguins, !body_mass_g) #everything except for body mass
```
...
```{r}
rename(penguins, body_mass = body_mass_g) #"new name" = "old name". Doesn't overwrite it 
```
...
```{r}
relocate(penguins, year) #moves variable to the front, depending on what you want viewers to focus on.
```
