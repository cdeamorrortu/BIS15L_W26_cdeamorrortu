...
```{r}
#setwd("/Users/cdeamorrortu/Documents/yourfolder") #uncomment and change to your path
```
...
```{r}
z = 10 #this will work, but will create problems eventually. always use "gets," never "equals"
```
...
```{r}
mean(c(2, 8, 2)) 
#c stands for 'combine,' and creates a vector
```

Why does this not work?
```{r}
#mean(2, 8, 2) doesn't work because we have not put the numbers into a vector
```
...
```{r}
mean_speed <- mean(c(venom_GT + chiron + veyron))/3
mean_speed
```
...
```{r}
my_missing <- NA #NA stands for a missing value
```
...
NAs override any of the other numbers in the dataset, and thus make the overall average of the set an NA as well.
