...
lat <- c(34.67, 41.80)
long <- c(-124.1, -115.5)
bbox <- make_bbox(long, lat, f = 0.03) #f makes a buffer for the box, value inputed is a percentage
```
...
```{r}
ggmap(map1) +
  geom_point(data = spiders, aes(longitude, latitude), size = 0.75, color = "pink", alpha = 0.8) +
...
spiders %>% 
  summarize(
  lon.min = min(longitude, na.rm = T), 
  lon.max = max(longitude, na.rm = T),
  lat.min = min(latitude, na.rm = T), 
  lat.max = max(latitude, na.rm = T)
  )
```
...
leaflet(spiders) %>% 
  ...
  fitBounds(lng1 = lon.min, lat1 = lat.min, lng2 = lon.max, lat2 = lat.max)
```
...
```{r message=FALSE, warning=FALSE}
grizzly <- clean_names(read_csv("data/bear-sightings.csv"))
summary(grizzly)
```
...
```{r}
bearsmap <- get_stadiamap(bearsbbox, maptype = "stamen_terrain", zoom = 2)
```
...
```{r}
ggmap(bearsmap) +
geom_point(data = grizzly, aes(longitude, latitude), size = 0.5, color = "firebrick")
```
